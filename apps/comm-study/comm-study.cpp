...
auto work(P &A, P &B, P &Y, int N)
{
   ...
   // init A and B separately - will it cause an H2D copy?
    ...
    sender auto s3 = then(just(), [&]{
        std::transform(std::execution::par_unseq, &A[0], &A[N], &B[0], &A[0], [&](T& ai, T& bi) { return ai + bi; });
        std::transform(std::execution::par_unseq, &A[0], &A[N], &B[0], &Y[0], [&](T& ai, T& bi) { return sqrt(pow(ai, 2) + pow(bi, 2))/(ai+bi); });
    })
    ...
    // get sum(Y) - wonder if there is another H2D as we only read it in the last step
    | then([&]{
        return std::reduce(std::execution::par_unseq, &Y[0], &Y[N], 0.0, std::plus<T>());
    });
