...
// Our operator
__device__ Real_t heat(const Real_t left, const Real_t middle, const Real_t right, const Real_t k, const Real_t dt,
                       const Real_t dx) {
    return middle + (k * dt / (dx * dx)) * (left - 2 * middle + right);
}

__global__ void heat_equation(Real_t* current, Real_t* next, std::size_t size, const Real_t k, const Real_t dt,
                              const Real_t dx) {
...
int benchmark(args_params_t const& args) {
    ...
    if (args.results) {
        ...
        cudaMemcpy(h_current, d_current, size * sizeof(Real_t), cudaMemcpyDeviceToHost);
